...
<head>
    ...
    <title>@yield('title','Dashboard') – AquaSync Admin</title>
    ...
    <style>
    :root {
        --sb-bg: #0f2942; --sb-hover: #1a3a5c; --sb-active: #1d6fa4; --sb-w: 260px;
        --primary: #1d6fa4; --primary-lt: #e8f4fd;
        --success: #2dc653; --warning: #f4a261; --danger: #e63946;
        --body-bg: #f4f7fb; --card: #fff;
        --text: #1a2b3c; --muted: #6b7c93; --border: #e2e8f0;
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .sb-badge{display:inline-block;margin-top:6px;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.8px;padding:3px 9px;border-radius:4px;background:rgba(29,111,164,.15);color:#6db8e8;border:1px solid rgba(29,111,164,.3)}
    ...
    .nav-link-a.active{background:var(--sb-active);color:#fff;box-shadow:0 2px 8px rgba(29,111,164,.38)}
    ...
    .ico-blue{background:rgba(29,111,164,.1);color:var(--primary)}
    ...
    .bdg-info{background:rgba(29,111,164,.1);color:var(--primary)}
    ...
    .finp:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(29,111,164,.1)}
    ...
    .btn-p:hover{background:#155e8e;color:#fff;transform:translateY(-1px)}
    ...
    .btn-d{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;background:#fff5f5;color:var(--danger);border:1.5px solid #fecaca;border-radius:var(--r-sm);font-size:.8rem;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;text-decoration:none;transition:all .15s}
    ...
    .toast-i.s{border-color:var(--success)}.toast-i.e{border-color:var(--danger)}.toast-i.w{border-color:var(--warning)}
...
<aside class="sb">
    ...
    <nav class="sb-nav">
        <div class="sb-sec">
            <a href="{{ route('admin.dashboard') }}" class="nav-link-a {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                <i data-lucide="layout-dashboard" class="nav-icon"></i> Dashboard
            </a>
        </div>
        <div class="sb-sec">
            <div class="sb-sec-lbl">Manajemen Utama</div>
            <a href="{{ route('admin.kolam.index') }}" class="nav-link-a {{ request()->routeIs('admin.kolam.*') ? 'active' : '' }}">
                <i data-lucide="grid-2x2" class="nav-icon"></i> Data Kolam
            </a>
            <a href="{{ route('admin.kualitas-air.index') }}" class="nav-link-a {{ request()->routeIs('admin.kualitas-air.*') ? 'active' : '' }}">
                <i data-lucide="activity" class="nav-icon"></i> Kualitas Air
            </a>
            <a href="{{ route('admin.jadwal-pakan.index') }}" class="nav-link-a {{ request()->routeIs('admin.jadwal-pakan.*') ? 'active' : '' }}">
                <i data-lucide="calendar-clock" class="nav-icon"></i> Jadwal Pakan
            </a>
            <a href="{{ route('admin.pengurasan.index') }}" class="nav-link-a {{ request()->routeIs('admin.pengurasan.*') ? 'active' : '' }}">
                <i data-lucide="droplets" class="nav-icon"></i> Pengurasan Air
            </a>
            <a href="{{ route('admin.panen.index') }}" class="nav-link-a {{ request()->routeIs('admin.panen.*') ? 'active' : '' }}">
                <i data-lucide="package" class="nav-icon"></i> Panen
            </a>
        </div>
        <div class="sb-sec">
            <div class="sb-sec-lbl">Sistem</div>
            <a href="{{ route('admin.laporan.index') }}" class="nav-link-a {{ request()->routeIs('admin.laporan.*') ? 'active' : '' }}">
                <i data-lucide="bar-chart-2" class="nav-icon"></i> Laporan
            </a>
            <a href="{{ route('admin.komentar-flag.index') }}" class="nav-link-a {{ request()->routeIs('admin.komentar-flag.*') ? 'active' : '' }}">
                <i data-lucide="flag" class="nav-icon"></i> Komentar & Flag
                @php $fc = \App\Models\KomentarFlag::where('status','menunggu')->count() @endphp
                @if($fc)<span class="nav-badge">{{ $fc }}</span>@endif
            </a>
            <a href="{{ route('admin.notifikasi.index') }}" class="nav-link-a {{ request()->routeIs('admin.notifikasi.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.akun.index') }}" class="nav-link-a {{ request()->routeIs('admin.akun.*') ? 'active' : '' }}">
                <i data-lucide="user-cog" class="nav-icon"></i> Akun
            </a>
        </div>
    </nav>
    <div class="sb-foot">
...
<div class="main-wrap">
    <header class="topbar">
        ...
        <div class="topbar-right">
            <a href="{{ route('admin.notifikasi.index') }}" class="btn-icon">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" style="min-width:176px;border-radius:10px;border:1px solid var(--border);box-shadow:var(--shadow-md);padding:8px;">
                    <li><span class="dropdown-item-text" style="font-size:.76rem;color:var(--muted);padding:5px 13px;">
                        {{ auth()->user()->name }}<br><strong style="color:var(--primary);">Admin</strong>
                    </span></li>
                    <li><hr class="dropdown-divider" style="margin:4px 0;"></li>
                    <li><a href="{{ route('admin.akun.index') }}" class="dropdown-item" style="font-size:.86rem;border-radius:6px;">
                        <i data-lucide="settings" style="width:13px;height:13px;margin-right:6px;"></i>Pengaturan
                    </a></li>
    ...
    <main class="page-body">
        ...
        <div class="toast-wrap" id="tw">
            ...
            @if(session('warning'))<div class="toast-i w"><i data-lucide="alert-triangle" style="width:17px;height:17px;color:var(--warning);flex-shrink:0;"></i>{{ session('warning') }}</div>@endif
        </div>
        @endif
        @yield('content')
    </main>
