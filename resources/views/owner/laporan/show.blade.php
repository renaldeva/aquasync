...
<div class="ph d-flex align-items-start justify-content-between">
    ...
    <div style="display:flex;gap:8px">
        <a href="{{ route('owner.laporan.index') }}" class="btn-s">
            <i data-lucide="arrow-left" style="width:15px;height:15px"></i> Kembali
        </a>
        <a href="{{ route('owner.laporan.download',$laporan) }}" class="btn-p">
...
<div class="card-a mb-3">
    <div style="display:flex;gap:24px;flex-wrap:wrap;padding-bottom:16px;border-bottom:1px solid var(--border);margin-bottom:16px">
        ...
        @php
            // 🔥 disesuaikan untuk owner (tanpa draft/rejected)
            $stMap = [
                'published' => 'bdg-aktif',
                'approved'  => 'bdg-aman'
            ];
        ...
        <div>
            <div style="font-size:.71rem;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:3px">
                {{ $label }}
            </div>
        ...
        <div>
            ...
            <span class="bdg {{ $stMap[$laporan->status]??'bdg-menunggu' }}">
    ...
    {{-- KONTEN LAPORAN --}}
    @if($laporan->konten)
    <h3 style="font-size:.9rem;font-weight:700;margin-bottom:14px">
        Ringkasan Data
    </h3>
    ...
    <div class="row g-3 mb-3">
        @foreach([
            ...
            ['pH Minimum',$s['min']??'–'],
            ['pH Maksimum',$s['max']??'–'],
            ['Total Bacaan',$s['total']??'–']
        ] as [$l,$v])
        <div class="col-6 col-md-3">
            <div style="background:var(--body-bg);border-radius:var(--r-sm);padding:14px;text-align:center">
                <div style="font-size:.71rem;color:var(--muted);text-transform:uppercase;margin-bottom:6px">
                    {{ $l }}
                </div>
    ...
    <div class="row g-3 mb-3">
        @foreach([
            ['Total Berat',number_format($laporan->konten['total_berat']).' kg'],
            ['Total Nilai','Rp '.number_format($laporan->konten['total_nilai']??0)]
        ] as [$l,$v])
        <div class="col-6">
            <div style="background:var(--body-bg);border-radius:var(--r-sm);padding:14px;text-align:center">
                <div style="font-size:.71rem;color:var(--muted);text-transform:uppercase;margin-bottom:6px">
    ...
    <div class="row g-3 mb-3">
        @foreach([
            ['Total Eksekusi',$laporan->konten['total_eksekusi']],
            ['Berhasil',$laporan->konten['sukses']??'–']
        ] as [$l,$v])
        <div class="col-6">
            <div style="background:var(--body-bg);border-radius:var(--r-sm);padding:14px;text-align:center">
                <div style="font-size:.71rem;color:var(--muted);text-transform:uppercase;margin-bottom:6px">
    ...
    <details style="margin-top:16px">
        <summary style="font-size:.82rem;font-weight:600;color:var(--muted);cursor:pointer">
            Lihat data mentah (JSON)
        </summary>
        <pre style="background:var(--body-bg);border-radius:var(--r-sm);padding:14px;font-size:.77rem;overflow-x:auto;margin-top:10px;font-family:'DM Mono',monospace">
{{ json_encode($laporan->konten, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE) }}
        </pre>
    </details>
